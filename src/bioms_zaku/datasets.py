...
EN: The bundled example data, available after `pip install` (v1.2). The files live in `bioms_zaku/examples` inside the installed
    ...
    Python:
        ...
        df = load_example("synthetic_400")            # pandas DataFrame
        folder = copy_examples("my_folder")           # CSVs + ready YAMLs, never overwriting (my_folder_2, …)
    Terminal:
        bioms-zaku examples                           # list
        bioms-zaku examples --copy [FOLDER]           # copy, then: cd FOLDER && bioms-zaku start example_data_400.csv
...
EXAMPLES: dict[str, dict] = {
    "synthetic_400": {"csv": "example_data_400.csv", "sep": ",", "decimal": ".", "kind": "synthetic",
                      "files": ["example_data_400.csv"]},
    "synthetic_8000": {"csv": "example_data.csv", "sep": ",", "decimal": ".", "kind": "synthetic",
                       "files": ["example_data.csv", "example_data_params.json", "example_quick.yaml", "example_full.yaml", "example_kg.yaml"]},
    "minimal": {"csv": "minimal_data.csv", "sep": ";", "decimal": ",", "kind": "synthetic",
                "files": ["minimal_data.csv", "minimal.yaml"]},
    "nhanes_diabetes_400": {"csv": "nhanes_diabetes_400.csv", "sep": ",", "decimal": ".", "kind": "real",
...
def list_examples() -> list[dict]:
    """EN: one dict per example: name, kind (synthetic | real), rows, columns, main file, companion files, description."""
    from .i18n import t
    d = examples_dir(); out = []
    for name, e in EXAMPLES.items():
        df = pd.read_csv(d / e["csv"], sep=e["sep"], decimal=e["decimal"])
        out.append({"name": name, "kind": e["kind"], "rows": int(len(df)), "columns": list(df.columns), "csv": e["csv"],
...
def copy_examples(dest: str | Path | None = None, names: list[str] | None = None) -> Path:
    """
    EN: copy the example files into `dest` (default ./zaku_exemplos); an existing non-empty folder is never touched — the copy
