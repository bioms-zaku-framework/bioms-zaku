...
SUGGEST = {
    ...
    # EN: optional columns that the catalog EQUATIONS need (age, sex, circumferences); suggested, never assumed
...
# EN: role flag → column name inside the catalog (inputs of the equations)
...
# IT: deve essere true solo se NESSUN target/controllo è calcolato dalle variabili mappate.
# catalog.include: "curated" = only methods whose primary source was critically read (default); "all" = every catalog
...
def init(csv: str, out: str | None = None, *, ask: Callable[[str, str | None], str] | None = None, map_flags: dict[str, str] | None = None,
    ...
    # EN: optional columns the catalog equations need; empty = not mapped (check will list which methods are skipped and why)
    groups: dict[str, str] = {}
    help_("w.help.groups")
    ...
    for role, gname in GROUP_ROLES.items():
        ...
        v = col(role, t("w.group_col", role=f"{role} ({t('w.role.' + role)})"), required=False,
                default=(None if dflt in taken else dflt) or "")          # EN: never suggest the target/control/covariate as a catalog input
        if v:
            groups[gname] = v
    help_("w.help.indep")
