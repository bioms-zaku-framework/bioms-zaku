...
def write_summary(out_dir: Path, cfg: dict, tables: dict[str, pd.DataFrame], manifest: dict) -> Path:
    ...
    for s in strata:
        ...
        if aud is not None and not aud.empty:
            ...
            for t in sorted(set(au.target)):
                ...
                L.append(f"- audit `{t}` vs `{at.control.iloc[0]}` ({at.metric.iloc[0]}, {at.estimator.iloc[0]}): "
                         f"SPECIFIC {vc.get('SPECIFIC', 0)} · MEASURES_CONTROL {vc.get('MEASURES_CONTROL', 0)} · INCONCLUSIVE {vc.get('INCONCLUSIVE', 0)}")
    ...
    if "sigma_transfer" in tables and not tables["sigma_transfer"].empty:
        ...
        L += ["## Σ transfer between strata", "", "| Σ from | observed in | type | pairs | median |err| | max |err| |", "|---|---|---|---|---|---|"]
        L += [f"| {x.sigma_from} | {x.observed_in} | {x.type} | {x.pairs} | {x.median_abs_err:.3f} | {x.max_abs_err:.3f} |" for x in t.itertuples()]
        L.append("")
