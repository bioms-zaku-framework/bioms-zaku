...
| 2 | `io` + `catalog` + parser seguro, com testes de contrato | ✔ 10/09: `src/bioms_zaku/{expr,catalog,io}.py`; `tools/migrate_catalog.py` (39→31, determinístico; removido em 14/09, migração concluída); `src/bioms_zaku/data/catalog_v1.json` (movido para dentro do pacote em 14/09) validado (vetores conferidos numericamente, 2 check_examples); 42 testes passando; `python -m build` + `twine check` OK. Correções feitas pelos testes: detecção de separador exigia conversão numérica; `inputs` faltava o grupo de ramo. Lima 2008 ganhou DOI 10.37527/2008.58.4.010 (Crossref, resolve). Aguardando revisão do Thalles |
...
| 6b | v0.5.2 autonomia (14/09) e v0.6 geometria alvo↔controle (14/09) | ✔ 14/09: sem dado local; `full`; exemplo reproduzível pelos parâmetros; threadpool em `run()`; §3.3 geometria (lição 12, testes de construção, portão no exemplo, ‡ nas figuras, `example_kg.yaml`). Aguardando inspeção das figuras pelo Thalles |
